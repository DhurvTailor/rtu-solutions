...
// Docxtemplater ka detailed error ek readable string mein convert karo.
// Docxtemplater jab MultiError throw karta hai to asli reason
// error.properties.errors array ke andar hota hai — har entry mein
// batata hai kaunsa tag/placeholder problem create kar raha hai.
function explainDocxError(err) {
  const errors = err?.properties?.errors;
  if (Array.isArray(errors) && errors.length > 0) {
    const details = errors
      .map((e) => {
        const tag = e?.properties?.xtag || e?.properties?.id || "unknown-tag";
        const explanation = e?.properties?.explanation || e?.message || "no explanation";
        return `[tag: ${tag}] ${explanation}`;
      })
      .join(" | ");
    return `Docx template error(s): ${details}`;
  }
  if (err?.properties?.explanation) {
    return `Docx template error: ${err.properties.explanation}`;
  }
  return err?.message || String(err);
}

// Ek docx template buffer ko diye gaye data se fill karo, filled buffer return karo.
// delimiters "{tag}" (single curly) rakhe hain kyunki templates isi format
// mein banaye gaye hain (nीचे comment dekho). Agar tum kabhi {{tag}} (double
// curly) style templates use karo to yahan delimiters override karna padega.
function renderDocxTemplate(templateBuffer, placeholderData) {
  // IMPORTANT: Docxtemplater template ko *construct* karte waqt hi parse/compile
  // kar leta hai (tags dhoondhta hai, syntax check karta hai) — render() call
  // hone se pehle hi. Isliye constructor aur render() dono ko try/catch mein
  // wrap karna zaroori hai, warna compile-time errors (unclosed tag, duplicate
  // tag, etc.) bina detail ke "Multi error" bankar bahar nikal jaate hain.
  try {
    const zip = new PizZip(templateBuffer);
    const doc = new Docxtemplater(zip, {
      paragraphLoop: true,
      linebreaks: true,
      // Templates {{tag}} (double curly) format use karte hain — YE ZAROORI
      // HAI kyunki report content mein JS code snippets bhi hote hain jo
      // normal single curly braces { } use karte hain (e.g. "const x = { }").
      // Agar delimiter single curly rakha jaye to Docxtemplater un code
      // braces ko bhi tag samajhne ki koshish karega aur crash karega.
      delimiters: { start: "{{", end: "}}" },
      nullGetter: (part) => {
        // Missing/undefined value par crash karne ke bajaye khaali string daalo,
        // aur console mein warn kar do taaki pata chale kaunsa field khaali gaya
        console.warn(`Report placeholder "${part.value}" ke liye data nahi mila, khaali chhod diya`);
        return "";
      },
    });
    ...
    // Detailed reason ke saath naya error throw karo taaki upar tak
    // (retry route, download route) exact wajah pahunche
    throw new Error(explainDocxError(err));
  }
}
...
// Template docx mein placeholder tags (single curly braces): {student_name}, {roll_no},
// {college_name}, {university_name}, {branch_name}, {degree_name},
// {semester_number}, {academic_session}, {training_company_name},
// {training_start_date}, {training_end_date}, {guide_name}, {hod_name},
// {subject_name}, {report_title}
...
export async function generateStudentReportDocx(studentReportId) {
  ...
  try {
    ...
    // Poora detail Vercel logs mein daalo (debugging ke liye)
    console.error(`Report generation failed for student_report ${studentReportId}:`, err);
    ...
    // Yahi readable message upar (retry route / download route) tak jaayega
