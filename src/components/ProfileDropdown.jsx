...
import Link from "next/link";
import {
  FaUser, FaGraduationCap, FaHistory,
  FaSignOutAlt, FaChevronDown, FaCrown,
  FaShieldAlt, FaShoppingBag,
} from "react-icons/fa";
...
export default function ProfileDropdown() {
  ...
  const menuItems = [
    ...
    {
      icon: <FaShoppingBag size={13} className="text-yellow-400" />,
      label: "My Purchases",
      desc: "Purchased premium PDFs",
      href: "/profile/purchases",
    },
    {
      icon: <FaHistory size={13} className="text-green-400" />,
      label: "Download History",
      desc: "All downloaded PDFs",
      href: "/profile/history",
    },
  ];
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-72 bg-[#0d1f3c] border border-white/10 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden z-50">
          ...
          {/* User Header */}
          <div className="px-4 py-4 bg-[#071A3D] border-b border-white/10">
            <div className="flex items-center gap-3">
              <Image
                src={session.user?.image || "/default-avatar.png"}
                alt={session.user?.name || "User"}
