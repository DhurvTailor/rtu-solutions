...
export default function Testimonials() {
  ...
  const getVisible = () => {
    ...
    return [
      ...
      comments[(current + 1) % comments.length],
    ];
  };
    ...
    <section className="py-20 sm:py-24 bg-white overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-[#071A3D] mt-2">
            Students YouTube Comments
          </h2>
          <div className="flex items-center justify-center gap-2 mt-2">
            <FaYoutube className="text-red-500" size={16} />
            <p className="text-gray-500 text-sm">RTU Solutions</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-16">
          {visible.map((c, i) => (
            <TestimonialCard key={`${c.id}-${i}`} c={c} />
          ))}
        </div>
        ...
        {comments.length > 1 && (
          <div className="flex items-center justify-center gap-6 mt-20">
            <button
              onClick={prev}
              aria-label="Previous"
              className="w-10 h-10 bg-[#071A3D] hover:bg-[#0d2a5e] rounded-full flex items-center justify-center transition"
            >
              <FaChevronLeft size={13} className="text-white" />
            </button>
            ...
            <div className="flex gap-2">
              {comments.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${
                    i === current ? "bg-[#E8700A] w-6" : "bg-gray-300 w-2"
                  }`}
                />
              ))}
            </div>
        ...
        <div className="text-center mt-10">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-[#E8700A] hover:bg-[#d9660a] text-white text-sm font-semibold px-6 py-2.5 rounded-full transition"
          >
            <FaYoutube size={16} /> Visit YouTube Channel
          </a>
        </div>
        ...
        {isAdmin && (
          <div className="text-center mt-6">
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  setComments(Array.isArray(updated) ? updated : []);
                  setCurrent(0);
                } catch { setSyncMsg("Sync fail ho gaya"); }
            ...
            {syncMsg && <p className="text-xs text-green-500 mt-1">{syncMsg}</p>}
          </div>
        )}
      </div>
    </section>
...
// ── Card — alag component taaki height consistent rahe ─────────────
function TestimonialCard({ c }) {
  return (
    <div className="relative pt-9 pb-10 px-3">

      {/* ── Polaroid — top right, rotated, overlapping border ── */}
      <div
        className="absolute -top-5 -right-3 z-30 bg-white shadow-lg p-1.5 pb-4 w-[76px]"
        style={{ transform: "rotate(8deg)", boxShadow: "3px 4px 10px rgba(0,0,0,0.25)" }}
      >
        {c.author_img ? (
          <img
            src={c.author_img}
          ...
          <div className="w-full aspect-square bg-[#071A3D] flex items-center justify-center text-white font-bold text-xl">
            {c.author_name?.charAt(0)?.toUpperCase() || "S"}
          </div>
        )}
        {/* Paperclip */}
        ...
        >
          <path
            d="M7 1 C3.5 1 1 3.5 1 7 L1 16 C1 17.1 1.9 18 3 18 C4.1 18 5 17.1 5 16 L5 7 C5 5.9 5.9 5 7 5 C8.1 5 9 5.9 9 7 L9 16 C9 18.2 7.2 20 5 20 C2.8 20 1 18.2 1 16 L1 7 C1 2.6 4.6 -1 9 -1"
            stroke="#9ca3af" strokeWidth="1.5" fill="none" strokeLinecap="round"
          />
        </svg>
      </div>

      {/* ── Outer black border frame (broken by quote marks) ── */}
      <div
        className="absolute inset-0 rounded-[36px] border-2 border-[#111] pointer-events-none z-10"
        style={{ margin: "0 6px" }}
      />

      {/* Top quote mark — sits ON the border line, breaking it */}
      <div className="absolute -top-[15px] left-8 z-20 bg-white px-2">
        <FaQuoteLeft className="text-[#111]" size={26} />
      </div>

      {/* Bottom quote mark — sits ON the border line, breaking it */}
      <div className="absolute -bottom-[9px] right-8 z-20 bg-white px-2">
        <FaQuoteRight className="text-[#111]" size={26} />
      </div>

      {/* ── Inner filled card ── */}
      <div
        className="relative z-10 bg-[#ff7515] rounded-2xl px-6 pt-9 pb-7 flex flex-col mx-4"
        style={{
          minHeight: "250px",
        }}
      >
        {/* Comment — fixed 5 lines, overflow hidden */}
        <p
          className="text-white text-sm leading-relaxed text-center flex-1"
          style={{
            display: "-webkit-box",
            WebkitLineClamp: 5,
        ...
        <div className="flex justify-center gap-1 mt-5">
          {[1, 2, 3, 4, 5].map((s) => (
            <FaStar key={s} className="text-white" size={16} />
          ))}
        </div>

        {/* Author */}
        <p className="text-center text-xs font-semibold text-white mt-3 truncate flex items-center justify-center gap-1">
          — {c.author_name}
          {c.likes > 0 && (
            <span className="ml-1 font-normal text-white/80 inline-flex items-center gap-1">
              <FaThumbsUp size={10} /> {c.likes}
            </span>
          )}
        </p>
      </div>
