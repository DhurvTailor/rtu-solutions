...
export async function GET() {
  try {
    ...
    const [rows] = await db.query(
      "SELECT * FROM student_information WHERE user_id = ?",
      [session.user.id]
    );

    return Response.json({ info: rows[0] || null });
  } catch (error) {
...
export async function POST(req) {
  try {
    ...
    const { phone_numberA, phone_numberB, college, city } = await req.json();

    if (!phone_numberA || phone_numberA.trim() === "") {
      return Response.json({ error: "Primary phone number zaroori hai" }, { status: 400 });
    }

    const userId = session.user.id;

    // Pehle check karo — existing record hai ya nahi
    const [existing] = await db.query(
      "SELECT id FROM student_information WHERE user_id = ?",
      [userId]
    );
    ...
    if (existing.length > 0) {
      ...
      await db.query(
        `UPDATE student_information 
         SET phone_numberA = ?, phone_numberB = ?, college = ?, city = ?, updated_at = NOW()
         WHERE user_id = ?`,
        [phone_numberA, phone_numberB || null, college || null, city || null, userId]
      );
    } else {
      // Insert karo
      await db.query(
        `INSERT INTO student_information (user_id, phone_numberA, phone_numberB, college, city)
         VALUES (?, ?, ?, ?, ?)`,
        [userId, phone_numberA, phone_numberB || null, college || null, city || null]
      );
    }

    return Response.json({ success: true, message: "Information save ho gayi!" });
  } catch (error) {
...
//     },
//     {
//       icon: <FaHistory className="text-green-400" />,
//       label: "History",
...
//       {/* Dropdown Panel */}
//       {open && (
//         <div className="absolute right-0 mt-3 w-72 bg-[#0d2454] border border-orange-500/20 rounded-xl shadow-2xl shadow-black/50 overflow-hidden z-50 animate-fade-in">
//           {/* User Header */}
