    ...
    <div className="min-h-screen py-10 px-4">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <QuickCard
            href="/profile/history"
            icon={<FaHistory className="text-2xl text-green-400" />}
            title="View History"
            desc="Recently viewed notes and papers"
            color="green"
          />
          <QuickCard
            ...
            icon={<FaSignOutAlt className="text-2xl text-red-400" />}
            title="Logout"
...
function QuickCard({ href, onClick, icon, title, desc, color }) {
  const borders = {
    blue: "hover:border-blue-500/40",
    pink: "hover:border-pink-500/40",
    green: "hover:border-green-500/40",
  ...
  const className = `bg-[#0d2454] border border-white/5 ${borders[color]} rounded-xl p-5 flex items-center gap-4 hover:bg-[#0d2454]/80 transition group w-full text-left`;
  ...
  // Logout jaise actions ke liye button (onClick), baaki sab ke liye Link (href)
  if (onClick) {
    return (
